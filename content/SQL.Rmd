---
title: "SQL en R con el paquete sqldf (en construcción)"
...
Como hemos visto en el apartado anterior, el asterisco (\*) sirve para seleccionar el conjunto de variables y observaciones del dataframe. Sin embargo, es problable que queramos **identificar únicamente las observaciones para un conjunto de variables** del dataset original. Para ello utilizaremos SELECT, que se asemeja en gran medida a la función `select()` del paquete {dplyr}. Pongamos que nos interesa únicamente estraer el **nombre del emperador, la dinastía, la fecha de nacimiento, la causa de la muerte, el asesino** de entre todas las variables del dataframe.
...
**En R**, como se ha señalado, indicando los mismos criterios de seleccion y utilizando la función `arrange()` del paquete {dplyr} obtenemos el mismo resultado.
...
Pongamos que nos interesa seleccionar aquellos **emperadores asesinados bien por su guardia pretoriana bien por su propio ejercito**:
...
Existen en SQL diversos procedimientos que podemos utilizar para extrer la información requerida del dataset original. Pongamos, por ejemplo, que nos interesa **únicamente el nombre de la agencia prestamista que se corresponde al mayor valor devuelto en el año 2015**. Para ello seleccionarmoes `agency_name` después de SELECT, para indicar la variable que queremos obtener, pero necesitamos otro SELECT para indicar las condiciones requeridas.
...
Para obtener el mismo resultado **en R** podemos utilizar la funcion `filter()` junto a `select()`.
...
Existen diversos **operadores que podemos utilizar en el lenguage SQL para filtrar las observaciones** de un dataframe. Por ejemplo:
