---
title: "Evolución VAB per cápita en Perú (1996-2016) con gganimate"
...
Una vez hemos preparado el dataset procedemos a realizar el **gráfico animado**. Llamaremos al gráfico `plot_peru` y lo guardamos con dicho nombre. Utilizaremos el color azul para identificar las regiones mayoritariamente costeras, el color verde las regiones selváticas y el color marrón indicará las regiones cuyo territorio es mayoritariamente montañoso. El nombre de los colores utilizados, seleccionados con la función `scale_fill_manual()` los he obtenido del link: http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf
...
Por último representamos el gráfico animado con `animate()`. Vemos claramente la totalidad de regiones y cómo van evolucionando en el *ranking* a lo largo de los años. La región de Moquegua mantiene su posición privilegiada a lo largo del periodo aunque se observa que otro conjunto de regiones, principalmente costeras (Lima, Arequipa, Ica o Tacna), van con el paso de los años acercando su nivel de renta per cápita con la de Moquegua. Por el contrario, en la parte inferior del *ranking*, un conjunto de regiones, principalmente regiones andinas, parecen consolidarse en posiciones más desfavorables con respecto al resto. 
...
Pongamos que queremos seleccionar únicamente los primeros puestos en el *ranking* podemos filtrar nuestras observaciones con la función `filter()` de dplyr. A modo de ejemplo **seleccionamos los diez primeros puestos** del *ranking* tal y como vemos en el código siguiente. 
...
Consecuentemente, utilizando el nuevo dataframe `Peru_rangos_top`, y realizando pequeñas modificaciones al código anterior podemos visualizar la **evolución de las regiones que ocupaban los primeros diez puestos** en el *ranking* según su VAB por habitante. En esta ocasión cambiamos también los colores del gráfico. Cómo hicimos previamente guardamos el nuevo gráfico, en este caso como `plot_peru_top` y posteriormente procedemos a realizar la animación con `animate()`. 
...
plot_peru_top <- ggplot(Peru_rangos_top, aes(rango, group = Nombre, 
  ...
  scale_y_continuous(labels = scales::comma) +
  scale_x_reverse() +
  scale_fill_manual(values=c("royalblue", "limegreen", "chocolate4")) +
  guides(color = FALSE, fill = FALSE) +
  transition_states(year, transition_length = 1, state_length = 1, wrap= F) + 
  labs(title = 'Perú. Regiones peruanas (Top 10) según su VAB per cápita',  
       subtitle  =  "Año : {closest_state}",
       caption  = "Miles de Soles constantes de 1994 | Fuente: INEI") +
  theme(
    legend.position="none",
    plot.title=element_text(size=18, hjust=0.5, face="bold", colour="grey20", vjust=-1),
    plot.subtitle=element_text(size = 26, hjust=0.5, face="plain", color="grey20"),
    plot.caption =element_text(size = 10, hjust=0.5, face="plain", color="grey20"),
    axis.line=element_blank(),
    plot.margin = margin(2,2, 2, 4, "cm"), 
    plot.background=element_blank(),
