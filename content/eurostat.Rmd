---
title: "Importar datos de Eurostat con el paquete eurostat()"
author: "Rubén F. Bustillo"
date: 2019-11-18
categories: ["R"]
tags: ["Eurostat", "tidyverse"]

---


```{r setup, include=FALSE}

knitr::opts_chunk$set(collapse = TRUE)

```

**Re**

En este post se explica las principales funcionalidades del paquete eurostat(), que sirve para importar información de dicho organismo. Eurostat es la Oficina de Estadística de la Comisión Europea.   

\

# Introducción

\

Eurostat es la Oficina de Estadística de la Comisión Europea, organismo que se encarga de producir datos, publicar estadísticas e indicadores sobre la Unión Europea promoviendo la armonización de los métodos estadísticos entre los Estados que la conforman. La función de este organismo es fundamental para realizar análisis y comparaciones entre países y entre regiones de la zona euro.

\

El paquete `eurostat` consiste en un conjunto de herramientas que tiene como finalidad facilitar la descarga de información desde la base de datos de Eurostat. Los autores de este paquete son Leo Lahti, Janne Huovari, Markus Kainu y Przemyslaw Biecek, quienes han puesto a disposición del público información que facilita su utilización y el aprendizaje de sus funcionalidades (véase el [artículo](https://journal.r-project.org/archive/2017/RJ-2017-019/RJ-2017-019.pdf) en R Journal, el [tutorial/ vignette](http://ropengov.github.io/eurostat/articles/eurostat_tutorial.html), el [cheatsheet del paquete](http://ropengov.github.io/eurostat/articles/cheatsheet.html), la documentación sobre sus [funcionalidades](http://ropengov.github.io/eurostat/reference/index.html) o el [documento](https://cran.r-project.org/web/packages/eurostat/eurostat.pdf) de CRAN.r-project ). 

\

La información de la presente entrada dificílmente podrá superar e incluso complementar a la disponible en los enlaces mencionados. No obstante cabe señalar que este post no pretende tener un carácter divulgativo sino que más bien nace con un objetivo de índole personal, en tanto en cuanto espero que sirva principalmente como proceso para familiarizarme con la utilización de dicho paquete. Dicho esto, si este post sirve de apoyo o ayuda a alguna persona interesada en aprender la utilización del paquete `eurostat` pues bienvenido sea. 

\

# El paquete eurostat()

\

Como siempre, en primer lugar cargamos los paquetes necesarios. Evidentemente cargamos el paquete `eurostat` y, adicionalmente, cargamos el paquete `tidyverse` cuyas funcionalidades se ajustan muy bien con el anterior. 

\

```{r,warning=FALSE, message=FALSE}

library(eurostat)
library(tidyverse)

```

\

Las funciones disponibles del paquete `eurostat` se pueden encontrar en el [link](http://ropengov.github.io/eurostat/reference/index.html) previamente mencionado o indicando:

\

```{r}

library(help= "eurostat")

```

\

Para ver el contenido (table of contents - TOC) del paquete podemos utilizar la función `get_eurostat_toc()`. Esta función nos da como resultado un tibble de 9863 observaciones y 8 variables (esta información se puede consultar en el siguiente [archivo de texto](http://ec.europa.eu/eurostat/estat-navtree-portlet-prod/BulkDownloadListing?sort=1&file=table_of_contents_en.txt)). A modo de ejemplo seleccionamos las primeras diez observaciones del mismo. 
 
\

```{r, warning=FALSE, message=FALSE}

contenido_eurostat <- get_eurostat_toc()

library(knitr)

kable(head(contenido_eurostat, n= 10))

```

\

Vemos que el dataset muestra el **título o nombre** de cada dataset, su **código**, si es folder o table (**type**) e información sobre la fecha de la **última actualización** u otros **cambios** o sobre las **fechas o periodos** que incluye cada dataset.

\

# Ejemplo 1. Homicidios

\

Para **buscar información en función de algun patrón o de alguna palabra clave** que sirva como pauta podemos utilizar la función `search_eurostat()`. A modo de ejemplo, y siendo un poco macabros, buscamos las TOC (tablas de contenidos) que estén relacionados con la palabra homicidio

\

```{r}

kable(search_eurostat("homicide"))

```

\

Examinaremos los homicidios intencionados en las grandes ciudades cuyo id es `crim_hom_ocit`. Para ello utilizamos la función `get_eurostat()`.

\

```{r, message=FALSE}

homicidios <- get_eurostat(id="crim_hom_ocit")
head(homicidios)
```

\

Utilizaremos la orden `time_format = "num"` dado que la información es anual y, por tanto, resulta más conveniente convertir la fecha a una variable numérica que continuar utilizando la variable `date`. 

\

```{r}

homicidios <- get_eurostat(id="crim_hom_ocit", time_format = "num")
head(homicidios)

```

\

Por otro lado, para remplazar los códigos del dataset por etiquetas (labels) usaremos la función `label_eurostat()` de la siguiente forma. De forma alternativa, al obtener el dataframe con la función `get_eurostat()` podemos utilizar `type= "label"` para obtener directamente las variables con etiquetas. 

\

```{r}

homicidios_labels <- label_eurostat(homicidios)

head(homicidios_labels)

str(homicidios_labels)

summary(homicidios_labels)

```

\

```{r}

label_eurostat_vars(names(homicidios_labels))

levels(homicidios_labels$unit)

levels(homicidios_labels$cities)

```

\

## Homicidios intencionados por 100.000 habitantes en 2008

A modo de ejemplo supongamos que queremos graficar el número de homicidios por cada cien mil habitantes en el año 2008 en las ciudades seleccionadas por Eurostat. 

\

```{r}

homicidios_labels_mil_2008 <- homicidios_labels %>%
  filter(unit == "Per hundred thousand inhabitants" & time == "2008" & values > 0.1) %>%
  mutate(cities = gsub(cities, pattern = " \\(.*", replacement="")) %>%
  mutate(cities = gsub(cities, pattern =" \\/.*", replacement ="")) %>%
  arrange(desc(values)) 

head(homicidios_labels_mil_2008) 

```

\

```{r, out.width= "70%", fig.width=7, fig.height=7, fig.align='center'}

ggplot(homicidios_labels_mil_2008, aes(x = reorder(cities, values), y = values)) +
  geom_col(color= "white", 
           fill= "#fc6721",
           alpha = 0.8,
           size = 2) +
  labs(title = "Homicidios por 100.000 habitantes en 2008",
       subtitle = "Selección de países",
       caption = "Fuente: Eurostat",
       y = "", x="") +
  coord_flip() +
  theme_minimal() +
  theme(panel.grid.major.x = element_blank(),
        axis.text.y= element_text(size = 12, hjust = 1),
        axis.text.x= element_text(size = 10),
        plot.title = element_text(size = 18, 
                                  face = "bold", 
                                  hjust = 0.5),
        plot.subtitle = element_text(size=14, 
                                     face = "plain", 
                                     hjust = 0.5))+
  geom_text(aes(label = values, y = values + 0.05), 
            hjust = 0,
            color = "grey40",
            size = 2.8)
  


```

\

## Evolución de los homicidios intencionados en Madrid, Paris y Londres

Veamos ahora la evolución de los homicidios (por 100.000 habitantes) en tres de las ciudades presentes en el dataframe: Madrid, Londres y Paris. El nuevo dataset lo llamaremos `homicidios_MPL` 

\

```{r}

homicidios_MLP <- homicidios_labels %>%
  filter(unit == "Per hundred thousand inhabitants") %>%
  mutate(cities = gsub(cities, pattern = " \\(.*", replacement="")) %>%
  filter(cities == "Madrid" | cities == "London" | cities == "Paris") 

head(homicidios_MLP) 

```

\  

```{r, out.width= "80%", fig.align='center', fig.width=8, fig.height=5}

ggplot(homicidios_MLP, aes(x= time, y= values, shape = cities, color = cities))+
  geom_point(size =2) +
  geom_line() +
  theme_minimal() +
  labs(title ="Homicidios intencionados por 100.000 habitantes",
       subtitle = "Selección de países",
       caption = "Fuente: Eurostat",
       x= "Year", y = "") 
  

```

\

# Ejemplo 2. Consumo de Alcohol

\

En primer lugar buscamos un dataframe que pueda servirnos para el objetivo marcado. El objetivo en este caso es examinar las diferencias en el consumo de alcohol por país.

\

```{r}

kable(search_eurostat("alcohol consumption"))

```

\

Seleccionamos el dataset que lleva como nombre *Frequency of alcohol consumption by sex, age and country of citizenship*. Al igual que sucedía con el ejemplo previo identificamos el formato para las fechas como valor númerico dado que los datos del dataset corresponden al año 2014. Para examinar las características del dataframe existen diversas alternativas:

\

```{r, message=FALSE}

consumo <- get_eurostat(id="hlth_ehis_al1c", time_format = "num")

head(consumo)

consumo_labels <- label_eurostat(consumo)

head(consumo_labels)

str(consumo_labels)

summary(consumo_labels)

```

\

El dataset contiene información del consumo según frecuencia (cada día, menos de una vez al mes, etc.), de sexo (femenino, masculino o total), por rangos de edad, por ciudadanía y por país. Para consultar el nombre completo de las variables del data set podemos utilizar la función `label_eurostat_vars(names())` y para comprobar las distintas categorías de las observaciones podemos examinar los levels como se expone a continuación. De esta forma podemos tener una imagen más adecuada del dataset sobre el que estamos trabajando:

\

```{r, message=FALSE}

label_eurostat_vars(names(consumo_labels))

levels(consumo_labels$unit)

levels(consumo_labels$frequenc)

levels(consumo_labels$sex)

levels(consumo_labels$age)

levels(consumo_labels$citizen)

levels(consumo_labels$geo)

levels(consumo$geo)

```

\

Resulta curioso comprobar que el dataset seleccionado, obtenido de Eurostat, no contiene información sobre Francia. 

\

A modo de ejercicios, partiendo del dataset obtenido de Eurostat, vamos a responder algunas preguntas:

\

### 1. ¿Cuál es la proporción de personas por país que consumen alcohol de forma diaria?

\

```{r, message=FALSE, warning=FALSE}

consumo <- get_eurostat(id="hlth_ehis_al1c", time_format = "num")

consumo_total <- consumo %>%
  filter(frequenc == "DAY") %>%
  filter(sex == "T") %>%
  filter(age == "TOTAL") %>%
  filter(citizen == "NAT") %>%
  filter(geo != "EU28")

head(consumo_total)

```

\ 

Realizamos el mapa utilizando la función `get_eurostat_geospatial()`, estableciendo 10 rangos para los valores y utilizando como [paleta de colores](http://rstudio-pubs-static.s3.amazonaws.com/5312_98fc1aba2d5740dd849a5ab797cc2c8d.html) la llamada *Spectral*. 

\

```{r, out.width = "90%", fig.align='center', message=FALSE, warning=FALSE}


consumo_total_mapa <- get_eurostat_geospatial(nuts_level = 0) %>%
  right_join(consumo_total) %>%
  mutate(categorias = cut_to_classes(values, n=10, decimals=1)) 

head(select(consumo_total_mapa, geo, values, categorias), 3)


ggplot(consumo_total_mapa, aes(fill=categorias)) +
  geom_sf(color = alpha("white", 1/2), alpha= 0.9) +
  xlim(c(-20, 44)) +
  ylim(c(35, 70)) +
  labs(title = "Frequency of alcohol consumption, 2014",
       subtitle = "Citizenship: National (has the citizenship of the reporting country)",
       caption = "Source: Eurostat",
       fill= "%")+
  theme_minimal() +
  theme(
    axis.line = element_blank(),
    axis.text = element_blank(),
    axis.title = element_blank(),
    axis.ticks = element_blank(),
    plot.background = element_rect(fill = "snow", color = NA),
    panel.background = element_rect(fill= "snow", color = NA),
    plot.title = element_text(size = 16, hjust = 0.5),
    plot.subtitle = element_text(size = 10, hjust = 0.5),
    plot.caption = element_text(size = 8, hjust = 1),
    legend.title = element_text(color = "grey40", size = 8),
    legend.text = element_text(color = "grey40", size = 7, hjust = 0),
    legend.position = c(0.93, 0.6),
    plot.margin = unit(c(0.5,2,0.5,1), "cm")) +
  scale_fill_brewer(palette= "Spectral")

```

\

### 2. ¿Cuál es la proporción de mujeres por país que consumen alcohol de forma diaria?

\

```{r, message=FALSE, warning=FALSE}

consumo <- get_eurostat(id="hlth_ehis_al1c", time_format = "num")

consumo_F <- consumo %>%
  filter(frequenc == "DAY") %>%
  filter(sex == "F") %>%
  filter(age == "TOTAL") %>%
  filter(citizen == "NAT") %>%
  filter(geo != "EU28")

```

\

```{r, out.width = "90%", fig.align='center', message=FALSE, warning=FALSE}


consumo_F_mapa <- get_eurostat_geospatial(nuts_level = 0) %>%
  right_join(consumo_F) %>%
  mutate(categorias = cut_to_classes(values, n=10, decimals=1)) 

head(select(consumo_F_mapa, geo, values, categorias), 3)


ggplot(consumo_F_mapa, aes(fill=categorias)) +
  geom_sf(color = alpha("white", 1/2), alpha= 0.9) +
  xlim(c(-20, 44)) +
  ylim(c(35, 70)) +
  labs(title = "Frequency of female alcohol consumption, 2014",
       subtitle = "Citizenship: National (has the citizenship of the reporting country)",
       caption = "Source: Eurostat",
       fill= "%")+
  theme_minimal() +
  theme(
    axis.line = element_blank(),
    axis.text = element_blank(),
    axis.title = element_blank(),
    axis.ticks = element_blank(),
    plot.background = element_rect(fill = "snow", color = NA),
    panel.background = element_rect(fill= "snow", color = NA),
    plot.title = element_text(size = 16, hjust = 0.5),
    plot.subtitle = element_text(size = 10, hjust = 0.5),
    plot.caption = element_text(size = 8, hjust = 1),
    legend.title = element_text(color = "grey40", size = 8),
    legend.text = element_text(color = "grey40", size = 7, hjust = 0),
    legend.position = c(0.93, 0.6),
    plot.margin = unit(c(0.5,2,0.5,1), "cm")) +
  scale_fill_brewer(palette= "Spectral")

```

\

### 3. ¿Cuál es la proporción de hombres por país que consumen alcohol de forma diaria?

\

```{r, message=FALSE, warning=FALSE}

consumo <- get_eurostat(id="hlth_ehis_al1c", time_format = "num")

consumo_M <- consumo %>%
  filter(frequenc == "DAY") %>%
  filter(sex == "M") %>%
  filter(age == "TOTAL") %>%
  filter(citizen == "NAT") %>%
  filter(geo != "EU28")

```

\

```{r, out.width = "90%", fig.align='center', message=FALSE, warning=FALSE}


consumo_M_mapa <- get_eurostat_geospatial(nuts_level = 0) %>%
  right_join(consumo_M) %>%
  mutate(categorias = cut_to_classes(values, n=10, decimals=1)) 

head(select(consumo_M_mapa, geo, values, categorias), 3)


ggplot(consumo_M_mapa, aes(fill=categorias)) +
  geom_sf(color = alpha("white", 1/2), alpha= 0.9) +
  xlim(c(-20, 44)) +
  ylim(c(35, 70)) +
  labs(title = "Frequency of male alcohol consumption, 2014",
       subtitle = "Citizenship: National (has the citizenship of the reporting country)",
       caption = "Source: Eurostat",
       fill= "%")+
  theme_minimal() +
  theme(
    axis.line = element_blank(),
    axis.text = element_blank(),
    axis.title = element_blank(),
    axis.ticks = element_blank(),
    plot.background = element_rect(fill = "snow", color = NA),
    panel.background = element_rect(fill= "snow", color = NA),
    plot.title = element_text(size = 16, hjust = 0.5),
    plot.subtitle = element_text(size = 10, hjust = 0.5),
    plot.caption = element_text(size = 8, hjust = 1),
    legend.title = element_text(color = "grey40", size = 8),
    legend.text = element_text(color = "grey40", size = 7, hjust = 0),
    legend.position = c(0.93, 0.6),
    plot.margin = unit(c(0.5,2,0.5,1), "cm")) +
  scale_fill_brewer(palette= "Spectral")

```

\

### 4. ¿Cuál es la proporción de población de entre 15 y 24 años por país que consumen alcohol de forma diaria?

\

```{r, message=FALSE, warning=FALSE}

consumo <- get_eurostat(id="hlth_ehis_al1c", time_format = "num")

consumo_1524 <- consumo %>%
  filter(frequenc == "DAY") %>%
  filter(sex == "T") %>%
  filter(age == "Y15-24") %>%
  filter(citizen == "NAT") %>%
  filter(geo != "EU28")

```

\

```{r, out.width = "90%", fig.align='center', message=FALSE, warning=FALSE}


consumo_1524_mapa <- get_eurostat_geospatial(nuts_level = 0) %>%
  right_join(consumo_1524) %>%
  mutate(categorias = cut_to_classes(values, n=10, decimals=1)) 

head(select(consumo_1524_mapa, geo, values, categorias), 3)


ggplot(consumo_1524_mapa, aes(fill=categorias)) +
  geom_sf(color = alpha("white", 1/2), alpha= 0.9) +
  xlim(c(-20, 44)) +
  ylim(c(35, 70)) +
  labs(title = "Frequency of alcohol consumption, Age: 15-24 yrs, 2014",
       subtitle = "Citizenship: National (has the citizenship of the reporting country)",
       caption = "Source: Eurostat",
       fill= "%")+
  theme_minimal() +
  theme(
    axis.line = element_blank(),
    axis.text = element_blank(),
    axis.title = element_blank(),
    axis.ticks = element_blank(),
    plot.background = element_rect(fill = "snow", color = NA),
    panel.background = element_rect(fill= "snow", color = NA),
    plot.title = element_text(size = 16, hjust = 0.5),
    plot.subtitle = element_text(size = 10, hjust = 0.5),
    plot.caption = element_text(size = 8, hjust = 1),
    legend.title = element_text(color = "grey40", size = 8),
    legend.text = element_text(color = "grey40", size = 7, hjust = 0),
    legend.position = c(0.93, 0.6),
    plot.margin = unit(c(0.5,2,0.5,1), "cm")) +
  scale_fill_brewer(palette= "Spectral")

```

\
