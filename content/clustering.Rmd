...
Para estimar las matrices de correlación de Baker y sus correspondientes coeficientes de correlación seguiriamos un procedimiento similar. 
...
Supongamos que para el año 2001 queremos clasificar el grupo de regiones peruanas en **seis clusters** con características similares (en este caso utilizamos el criterio de los promedios que viene por defecto). Para ello indicaremos `k=6`, corrspondiente al número de conglomerados, e indicamos el color que queremos utilizar para diferenciarlos gráficamente en nuestro dendrograma. 
...
Además de las arriba expuestas, con el paquete `fviz_dend()` existen otras opciones que pueden resultarnos de interés a la hora de realizar dendrogramas. Véamos algunos ejemplos:
...
En primer lugar, como vimos, obtenemos la distancia euclídea, tal y como expusimos al inicio del post. Recordemos que la distancia euclídea es la que viene determinada por defecto al usar la función `dist()`. En prmer lugar estimamos las distancias para el año 2001.
...
Supongamos que queremos agrupar las regiones peruanas en 6 conglomerados o clusters. En este sentido vemos que el resultado obtenido agrupa en un primer grupo 16 regiones del país (que se corresponde al dendragrama realizado anteriormente), un segundo conglomerado se conforma de 4 regiones y cuatro conglomerados muestran características diferenciadas que se configuran como únicos elementos de sus respectivos conglomerados. 
