...
Si queremos convertir el mapa 3D anterior a un choropleth mapa habitual que muestre los valores indicados en una escala de valores podemos realizar la siguiente operación:
...
Antes de proceder a realizar el mapa conviene reconfigurar el dataframe con el objetivo de hacerlo más funcional para su análisis. Utilizamos la función `gather()` del paquete `tidyverse`. 
...
Una vez tenemos el dataframe con la información del porcentage de la inversión en I+D en algunos países seleccionados procedemos a realizar el mapa 3d utilizando la función `e_map_3d()`. De esta forma podemos ver de forma sencilla el valor de dicha variable para cada país en cada uno de los años del periodo escogido así como la evolución temporal de dicho indicador.
